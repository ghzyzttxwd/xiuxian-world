...
const newDecision="const loadout=s.player.artifactLoadout||{},preparedRealm33Sword=s.player.realmIndex===33&&s.player.daoPath==='sword'&&loadout.assault==='item-v37-lawcleaver-sword'&&loadout.guard==='item-v32-swordguard-wheel'&&loadout.natal==='item-v32-sevenstar-swordcase',marrowSerpentChallenge=preferWin&&preparedRealm33Sword&&enemyRealm===34&&c.enemy?.kind==='祖脉异兽';if(marrowSerpentChallenge){realm34MarrowChallenges++;if(realm34MarrowChallenges<=8)console.log('V310_FULLRUN_V13_MARROW_CHALLENGE',JSON.stringify({count:realm34MarrowChallenges,enemy:c.enemy?.name,enemyRealm,playerRealm:s.player.realmIndex,hpRatio:Number(hpRatio.toFixed(3)),location:s.player.location,loadout:{assault:loadout.assault,guard:loadout.guard,natal:loadout.natal},actions}))}if(!preferWin||(enemyRealm>s.player.realmIndex&&!marrowSerpentChallenge)||hpRatio<.28){if(DAO_PATH==='sword'&&enemyRealm>s.player.realmIndex&&hpRatio>=.4&&(s.player.activeSkillIds||[]).includes('spell-v36-sword-space-step')){const cc=combat(),row=registry.spells['spell-v36-sword-space-step'],cd=Number(cc?.cooldowns?.['spell-v36-sword-space-step']||0);if(cc&&cd<=0&&!(cc.v36SpaceShift>0)&&cc.playerQi>=Number(row?.qi||0)){spendAction('combat-escape-shift',()=>invoke('combatAction','skill:spell-v36-sword-space-step'));continue}}spendAction('combat-flee',()=>invoke('combatAction','flee'));continue}";
...
console.log('V310_FULLRUN_V13_FINAL_RUNNER_PASS '+JSON.stringify({preparedRealm33SwordMayFightRealm34AncestralBeast:true,unrelatedHigherRealmSwordShiftFleePreserved:true,actualDropTableRoutingPreserved:true,noGameplayMutation:true,finalRunner:finalRunnerPath.pathname}));
