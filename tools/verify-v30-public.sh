...
EXPECT_GAME='5d2cf4cd48e9a485457546fbed222ed21b70a7258895dfde8965bffb08f9979d'
...
DIAG='/tmp/V30_PUBLIC_DIAGNOSTIC.txt'
OKFILE='/tmp/V30_PUBLIC_OK'
...
write_diag(){
  local status="$1" attempt="$2"
  {
    ...
    echo "index_v30=$index_v30"; echo "app_v30=$app_v30"; echo "game_v30=$game_v30"; echo "schema27=$schema27"; echo "sw_v30=$sw_v30"
...
for i in $(seq 1 "$ATTEMPTS"); do
  index_code=$(fetch_asset "${BASE}?v=3001&diag=${BASE_SHA}-${i}" /tmp/public-index.html /tmp/curl-index.err)
  app_code=$(fetch_asset "${BASE}app.js?v=3001&diag=${BASE_SHA}-${i}" /tmp/public-app.js /tmp/curl-app.err)
  game_code=$(fetch_asset "${BASE}src/game-v30.js?v=3001&diag=${BASE_SHA}-${i}" /tmp/public-game.js /tmp/curl-game.err)
  sw_code=$(fetch_asset "${BASE}sw.js?v=3001&diag=${BASE_SHA}-${i}" /tmp/public-sw.js /tmp/curl-sw.err)
  ...
  index_v30=0; app_v30=0; game_v30=0; schema27=0; sw_v30=0; game_sha='missing'; game_sha_ok=0; icon512_sha='missing'; icon512_ok=0; icon192_sha='missing'; icon192_ok=0; manifest_ok=0; regression_ok=0
  [ -f /tmp/public-index.html ] && grep -q 'V3.0 · 成长数据底座' /tmp/public-index.html && index_v30=1
  [ -f /tmp/public-app.js ] && grep -q 'src/game-v30.js?v=3001' /tmp/public-app.js && grep -q "gameplayVersion:'3.0.0'" /tmp/public-app.js && app_v30=1
  [ -f /tmp/public-game.js ] && grep -q "const VERSION='3.0.0'" /tmp/public-game.js && game_v30=1
  [ -f /tmp/public-game.js ] && grep -q 'const SAVE_SCHEMA_VERSION=27' /tmp/public-game.js && schema27=1
  [ -f /tmp/public-sw.js ] && grep -q 'taixuan-v3.0.0-growth-data-foundation-3001' /tmp/public-sw.js && sw_v30=1
  ...
  if [ "$index_v30" = 1 ] && [ "$game_v30" = 1 ] && [ "$schema27" = 1 ] && [ "$game_sha_ok" = 1 ]; then INDEX_PATH=/tmp/public-index.html GAME_PATH=/tmp/public-game.js node tests/regression-v30.mjs >/tmp/public-regression-v30.log 2>&1 && regression_ok=1; fi

  status=FAIL
  if [ "$index_code" = 200 ] && [ "$app_code" = 200 ] && [ "$game_code" = 200 ] && [ "$sw_code" = 200 ] && [ "$manifest_code" = 200 ] && [ "$icon512_code" = 200 ] && [ "$icon192_code" = 200 ] && [ "$index_v30" = 1 ] && [ "$app_v30" = 1 ] && [ "$game_v30" = 1 ] && [ "$schema27" = 1 ] && [ "$sw_v30" = 1 ] && [ "$game_sha_ok" = 1 ] && [ "$icon512_ok" = 1 ] && [ "$icon192_ok" = 1 ] && [ "$manifest_ok" = 1 ] && [ "$regression_ok" = 1 ]; then status=PASS; fi
  write_diag "$status" "$i"
