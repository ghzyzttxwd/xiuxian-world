...
for(const p of requiredPaths){
 ...
 paths[p]={file,seed:r.seed,actions:Number(r.actions),legalCallCount:Number(r.legalCallCount||0),finalAge:Number(f.age),finalManual:f.manual||null,battleWins:Number(f.battleWins||0),battleLosses:Number(f.battleLosses||0),lifeCycles:Number(r.proof?.lifeCycles||0),totalDeaths:Number(r.proof?.totalDeaths||0),realmAges:{r14:realmAge(r,14),r19:a19,r23:a23,r30:a30,r34:a34,r37:a37,r39:a39},timeSpans:{nascentToDeification:earlyMajorSpan,unityToMahayana:lateMajorSpan,mahayana:mahayanaSpan},finalResources:{stones:Number(f.stones||0),rareMaterials:Number(f.rareMaterials||0),originInsight:Number(f.originInsight||0),worldAuthority:Number(f.worldAuthority||0),natalMarks:Number(f.natalMarks||0)}};
}
...
for(const p of requiredPaths)assert(uniqueFastestCount[p]<dominanceHardLimit,`V3.10 route dominance detected: ${p} is uniquely fastest at ${uniqueFastestCount[p]}/${dominanceCheckpoints.length} major checkpoints; hard limit is ${dominanceHardLimit-1}`);
const summary={status:'PASS',gameplay_version:'3.10.0',build:'31001',paths,fourPathsComplete:true,freshSaveAll:true,noRechargeAll:true,legalGameplayOnlyAll:true,ascensionCompleteAll:true,highRealmTimeGrowthAll:true,terminalManualVariety:manuals.size,actionSpreadRatio:Number(actionSpreadRatio.toFixed(3)),actionSpreadAssessment:actionSpreadRatio<=2?'balanced-within-2x':actionSpreadRatio<=3?'review-2x-to-3x':'fail-over-3x',routeDominance:{checkpoints:dominanceCheckpoints,hardLimit:dominanceHardLimit,uniqueFastestByRealm,uniqueFastestCount,noNearTotalRouteDominance:true}};
