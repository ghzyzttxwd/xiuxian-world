...
// Reuse the already-supported one-shot hp/qi semantics.
replaceOnce(
...
// V33 breakthrough helpers already use effect.buff.breakthrough. Keep the V38 magnitudes exactly
// as authored and give them the same 25-day preparatory window used by 归一破境丹.
replaceOnce(
...
// 本命融源丹 is the only recipe using artifactWarmth. Apply it to the existing current natal
// artifact, respecting quality/toxicity scaling and the same 100-point warmth cap as normal warming.
...
// Fail closed against the exact inert V38 schemas that motivated this repair.
for(const stale of [
