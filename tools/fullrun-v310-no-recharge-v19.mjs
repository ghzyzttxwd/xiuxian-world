...
// invoked v38PrepareTribulation() without returning. V19 changes itinerary only: after each batch
// of normal material procurement, return to the required terrace before the normal prep action.
let v18=fs.readFileSync(v18Path,'utf8');
v18=replaceOnce(
 v18,
 "await import(finalRunnerPath.href+'?v18final='+Date.now());",
 "// v19 executes the final runner after fixing tribulation-prep return travel.",
...
console.log('V310_FULLRUN_V19_FINAL_RUNNER_PASS '+JSON.stringify({tribulationPrepReturnsToTerrace:true,normalMaterialProcurementPreserved:true,normalPrepApiPreserved:true,noDirectResourceInjection:true,noGameplayMutation:true,generatedRunnerSyntaxChecked:true,finalRunner:finalRunnerPath.pathname}));
