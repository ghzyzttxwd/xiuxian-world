...
function factionContractCooldown(id){const p=state.player,last=(p.factionLastContractDay&&p.factionLastContractDay[id])||0;return Math.max(0,10-(dayNumber()-last))}
...
`;
must("function daoMapBadge(location){return daoWorldResonance(location)?'<span class=\"pill\">道途共鸣</span>':''}\nconst ENEMIES=[",
     "function daoMapBadge(location){return daoWorldResonance(location)?'<span class=\"pill\">道途共鸣</span>:''}".replace("><span",">")+"\n"+factionSystem+'const ENEMIES=[','faction system');

// Correct the daoMapBadge anchor insertion without altering its V2.2 behavior.
src=src.replace("function daoMapBadge(location){return daoWorldResonance(location)?'<span class=\"pill\">道途共鸣</span>:''}","function daoMapBadge(location){return daoWorldResonance(location)?'<span class=\"pill\">道途共鸣</span>':''}");
