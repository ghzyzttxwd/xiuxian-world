...
const unityBefore="function ensureUnity(n){ensureLaw(Math.min(35,Math.max(20,n)));let guard=0;while((state().player.v37Unity||0)<n){if(++guard>MAX_FARM_ACTIONS)fail('unity-deadlock',{target:n});ensureNamed('mat-v37-law-crystal',1);ensureNamed('mat-v37-soul-covenant-stone',1);if(!goAny(['归一圣墟','万象法坛','天衡战城']))fail('unity-location-unreachable',{target:n});const before=state().player.v37Unity;const r=spendAction('integrate-unity',()=>invoke('v37IntegrateUnity'));heal();if(!r?.ok||state().player.v37Unity<=before)fail('unity-action-blocked',{result:r,target:n})}}";
const unityAfter="function ensureUnity(n){ensureLaw(Math.min(35,Math.max(20,n)));ensureSwordEscapeSkill();let guard=0;while((state().player.v37Unity||0)<n){if(++guard>MAX_FARM_ACTIONS)fail('unity-deadlock',{target:n});tryAuctionMaterial('mat-v37-law-crystal',1,24);ensureNamed('mat-v37-law-crystal',1);tryAuctionMaterial('mat-v37-soul-covenant-stone',1,48);ensureNamed('mat-v37-soul-covenant-stone',1);if(!goAny(['归一圣墟','万象法坛','天衡战城']))fail('unity-location-unreachable',{target:n});const before=state().player.v37Unity;const r=spendAction('integrate-unity',()=>invoke('v37IntegrateUnity'));heal();if(!r?.ok||state().player.v37Unity<=before)fail('unity-action-blocked',{result:r,target:n})}}";
runner=replaceOnce(runner,unityBefore,unityAfter,'prefer normal auction resources during ordinary unity progression');

const natalMarksBefore="function ensureNatalMarks(n){ensureNatalArtifact();let guard=0;while((state().player.v38NatalOriginMarks||0)<n){if(++guard>24)fail('natal-origin-mark-loop',{target:n});ensureNamed('mat-v38-natal-source-crystal',1);ensureNamed('mat-v38-origin-crystal',1);ensureNamed('mat-v38-origin-gold',1);ensureSpace(60);if(!goAny(['天穹祖脉','九霄劫台']))fail('natal-origin-location-unreachable',{target:n});const before=state().player.v38NatalOriginMarks;const r=spendAction('temper-natal-origin',()=>invoke('v38TemperNatalOrigin'));heal();if(!r?.ok||state().player.v38NatalOriginMarks<=before)fail('natal-origin-action-blocked',{result:r,target:n})}}";
const natalMarksAfter="function ensureNatalMarks(n){ensureNatalArtifact();ensureSwordEscapeSkill();let guard=0;while((state().player.v38NatalOriginMarks||0)<n){if(++guard>24)fail('natal-origin-mark-loop',{target:n});tryAuctionMaterial('mat-v38-natal-source-crystal',1,72);ensureNamed('mat-v38-natal-source-crystal',1);tryAuctionMaterial('mat-v38-origin-crystal',1,72);ensureNamed('mat-v38-origin-crystal',1);ensureNamed('mat-v38-origin-gold',1);ensureSpace(60);if(!goAny(['天穹祖脉','九霄劫台']))fail('natal-origin-location-unreachable',{target:n});const before=state().player.v38NatalOriginMarks;const r=spendAction('temper-natal-origin',()=>invoke('v38TemperNatalOrigin'));heal();if(!r?.ok||state().player.v38NatalOriginMarks<=before)fail('natal-origin-action-blocked',{result:r,target:n})}}";
runner=replaceOnce(runner,natalMarksBefore,natalMarksAfter,'prefer normal auction for natal-origin inputs while keeping origin-gold dangerous');
...
if(!runner.includes("tryAuctionMaterial('mat-v37-soul-covenant-stone',1,48)"))throw new Error('final runner still explores first for ordinary unity covenant stones');
if(!runner.includes("tryAuctionMaterial('mat-v38-natal-source-crystal',1,72)"))throw new Error('final runner still explores first for natal-source crystal');
if(!runner.includes("tryAuctionMaterial('mat-v38-origin-crystal',1,72)"))throw new Error('final runner still explores first for origin crystal during natal tempering');
if(!runner.includes("ensureNamed('mat-v38-origin-gold',1);"))throw new Error('final runner no longer preserves dangerous origin-gold acquisition');
