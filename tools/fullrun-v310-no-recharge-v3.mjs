...
const legalEconomyBefore="'contentRegistrySnapshot','v31CatalogSnapshot','learnV31Manual'";
const legalEconomyAfter="'contentRegistrySnapshot','v35EconomySnapshot','v35ListingRegistry','v35Quote','v35Trade','v31CatalogSnapshot','learnV31Manual'";
const namedAnchor="function ensureNamed(id,n){if(materialCount(id)>=n)return;";
const auctionHelper="const AUCTION_MATERIAL_IDS=new Set(['mat-v36-space-crystal','mat-v36-void-sand','mat-v36-void-essence','mat-v37-law-crystal','mat-v37-domain-sand','mat-v38-origin-crystal','mat-v38-natal-source-crystal']);function tryAuctionMaterial(id,n,maxCycles=24){if(materialCount(id)>=n)return true;if(!AUCTION_MATERIAL_IDS.has(id)||state().player.realmIndex<25)return false;if(!goTo('苍梧郡城'))return false;let cycles=0;while(materialCount(id)<n){const lots=Object.values(invoke('v35ListingRegistry')||{}).filter(x=>x&&x.shopId==='shop-cangwu-auction'&&x.kind==='material'&&x.refId===id&&Number(x.stock||0)>0);if(lots.length){const lot=lots[0],q=invoke('v35Quote',lot.id,'buy',1);if(q&&q.total>0){earnStones(q.total+20);if(!goTo('苍梧郡城'))fail('auction-return-unreachable',{id,target:n});const before=materialCount(id);const r=spendAction('auction-buy:'+id,()=>invoke('v35Trade',lot.id,'buy',1));if(r?.ok&&materialCount(id)>before){console.log('V310_FULLRUN_MATERIAL',JSON.stringify({source:'auction',id,count:materialCount(id),target:n,price:q.total,actions}));continue}}}if(cycles++>=maxCycles)break;const cycle=Number(invoke('v35EconomySnapshot').auctionCycle)||0;let wait=0;while((Number(invoke('v35EconomySnapshot').auctionCycle)||0)===cycle){if(++wait>35)fail('auction-cycle-wait-loop',{id,target:n,cycle});act('rest',false)}}return materialCount(id)>=n}function ensureNamed(id,n){if(materialCount(id)>=n)return;";
const voidBefore="function ensureVoidEssence(n){let guard=0;while(materialCount('mat-v36-void-essence')<n){";
const voidAfter="function ensureVoidEssence(n){if(tryAuctionMaterial('mat-v36-void-essence',n,24))return;let guard=0;while(materialCount('mat-v36-void-essence')<n){";
const writeAnchor="fs.writeFileSync(outPath,src);";
const nestedPatch=`mustReplace(\n ${JSON.stringify(rareBefore)},\n ${JSON.stringify(rareAfter)},\n 'prefer Qingyun rare-material exchange for sword path'\n);\nmustReplace(${JSON.stringify(coreStockBefore)},${JSON.stringify(coreStockAfter)},'consume stocked core materials');\nmustReplace(${JSON.stringify(nascentStockBefore)},${JSON.stringify(nascentStockAfter)},'consume stocked nascent materials');\nmustReplace(${JSON.stringify(deificationStockBefore)},${JSON.stringify(deificationStockAfter)},'consume stocked deification materials');\nmustReplace(${JSON.stringify(legalEconomyBefore)},${JSON.stringify(legalEconomyAfter)},'allow normal V3.5 economy gameplay surface');\nmustReplace(${JSON.stringify(namedAnchor)},${JSON.stringify(auctionHelper)},'add normal auction material strategy');\nmustReplace(${JSON.stringify(voidBefore)},${JSON.stringify(voidAfter)},'prefer normal auction for refining-void essence');\nmustReplace(\n "dom.window.Math.random=seededRandom(SEED);",\n "dom.window.Date.now=()=>SEED;dom.window.Math.random=seededRandom(SEED);",\n 'deterministic fresh-save clock'\n);\nif(!src.includes("spendAction('sect-exchange-rare-ui'"))throw new Error('Qingyun rare-material exchange strategy missing');\nif(src.includes('ensureRelic(state().player.relicFragments+2)'))throw new Error('nascent stock-preservation bug survived');\nif(!src.includes("source:'auction'"))throw new Error('normal auction material strategy missing');\nif(!src.includes("tryAuctionMaterial('mat-v36-void-essence',n,24)"))throw new Error('refining-void auction strategy missing');\nfs.writeFileSync(outPath,src);`;
mustReplace(writeAnchor,nestedPatch,'inject runner-level inventory, economy, deterministic clock and Qingyun rare strategy');
...
if(!src.includes("allow normal V3.5 economy gameplay surface"))throw new Error('normal economy nested transform missing');
if(!src.includes("prefer normal auction for refining-void essence"))throw new Error('refining-void auction nested transform missing');
