...
(async()=>{
  try{
    ...
    await loadPatch('./v12-patch.js?v=1201','__TAIXUAN_PATCH_V12__','V1.2补丁');
    ...
    src=window.__TAIXUAN_PATCH_V03__(src);src=window.__TAIXUAN_PATCH_V04__(src);src=window.__TAIXUAN_PATCH_V05__(src);src=window.__TAIXUAN_PATCH_V06__(src);src=window.__TAIXUAN_PATCH_V07__(src);src=window.__TAIXUAN_PATCH_V08__(src);src=window.__TAIXUAN_PATCH_V09__(src);src=window.__TAIXUAN_PATCH_V10__(src);src=window.__TAIXUAN_PATCH_V11__(src);src=window.__TAIXUAN_PATCH_V12__(src);
    if(!src.includes("const VERSION='1.2.0'"))throw new Error('V1.2最终源码版本断言失败');
    if(!src.includes('spawnSecretRealm')||!src.includes('enterSecretRealm')||!src.includes('claimSecretRealmCore')||!src.includes('decipherRelic')||!src.includes('renderSecretRealm'))throw new Error('V1.2秘境模块断言失败');
    document.title='太玄界 · 修仙大世界 V1.2';
    const sub=document.querySelector('.start-box > p');if(sub)sub.textContent='修仙大世界 · V1.2';
    const ver=document.querySelector('.start-box .version');if(ver)ver.textContent='V1.2 · 秘境与机缘';
    window.__TAIXUAN_BUILD__={version:'1.2.0',milestone:'secret-realm',patches:['0301','0401','0501','0601','0701','0801','0901','1001','1104','1201']};
    (0,eval)(src);
