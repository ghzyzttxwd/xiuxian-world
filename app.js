...
(async()=>{
  try{
    ...
    src=window.__TAIXUAN_PATCH_V03__(src);src=window.__TAIXUAN_PATCH_V04__(src);src=window.__TAIXUAN_PATCH_V05__(src);src=window.__TAIXUAN_PATCH_V06__(src);src=window.__TAIXUAN_PATCH_V07__(src);src=window.__TAIXUAN_PATCH_V08__(src);src=window.__TAIXUAN_PATCH_V09__(src);src=window.__TAIXUAN_PATCH_V10__(src);
    if(!src.includes("const VERSION='1.0.0'"))throw new Error('V1.0最终源码版本断言失败');
    if(!src.includes('buildOrUpgradeDwelling')||!src.includes('retreatSevenDays')||!src.includes('plantHerbPlot')||!src.includes('renderDwelling'))throw new Error('V1.0洞府模块断言失败');
    document.title='太玄界 · 修仙大世界 V1.0';
    const sub=document.querySelector('.start-box > p');if(sub)sub.textContent='修仙大世界 · V1.0';
    const ver=document.querySelector('.start-box .version');if(ver)ver.textContent='V1.0 · 洞府与闭关闭环';
    window.__TAIXUAN_BUILD__={version:'1.0.0',milestone:'dwelling-loop',patches:['0301','0401','0501','0601','0701','0801','0901','1001']};
    (0,eval)(src);
