...
(async()=>{
  try{
    ...
    src=window.__TAIXUAN_PATCH_V03__(src);src=window.__TAIXUAN_PATCH_V04__(src);src=window.__TAIXUAN_PATCH_V05__(src);src=window.__TAIXUAN_PATCH_V06__(src);src=window.__TAIXUAN_PATCH_V07__(src);src=window.__TAIXUAN_PATCH_V08__(src);
    if(!src.includes("const VERSION='0.8.0'"))throw new Error('V0.8最终源码版本断言失败');
    if(!src.includes('giftNPC')||!src.includes('processSocialEvents')||!src.includes('data-gift-herb'))throw new Error('V0.8关系模块断言失败');
    document.title='太玄界 · 修仙大世界 V0.8';
    const sub=document.querySelector('.start-box > p');if(sub)sub.textContent='修仙大世界 · V0.8';
    const ver=document.querySelector('.start-box .version');if(ver)ver.textContent='V0.8 · 人情与恩怨闭环';
    window.__TAIXUAN_BUILD__={version:'0.8.0',milestone:'relations-loop',patches:['0301','0401','0501','0601','0701','0801']};
    (0,eval)(src);
