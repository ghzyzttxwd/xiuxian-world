...
(async()=>{
  try{
    ...
    src=window.__TAIXUAN_PATCH_V03__(src);src=window.__TAIXUAN_PATCH_V04__(src);src=window.__TAIXUAN_PATCH_V05__(src);src=window.__TAIXUAN_PATCH_V06__(src);
    if(!src.includes("const VERSION='0.6.0'"))throw new Error('V0.6最终源码版本断言失败');
    if(!src.includes('renderAlchemy')||!src.includes('brewAlchemy')||!src.includes('qiPills'))throw new Error('V0.6炼丹模块断言失败');
    document.title='太玄界 · 修仙大世界 V0.6';
    const sub=document.querySelector('.start-box > p');if(sub)sub.textContent='修仙大世界 · V0.6';
    const ver=document.querySelector('.start-box .version');if(ver)ver.textContent='V0.6 · 炼丹与资源闭环';
    window.__TAIXUAN_BUILD__={version:'0.6.0',milestone:'alchemy-loop',patches:['0301','0401','0501','0601']};
    (0,eval)(src);
