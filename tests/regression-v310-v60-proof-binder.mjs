...
function result(dao,index){
 const realmLog=checkpoints.map(i=>({realmIndex:i,realm:`R${i}`,age:baseAge[i]}));
 return {status:'PASS',seed:31006701+index,daoPath:dao,actions:180000+index*5000,maxRealm:39,realmLog,legalCallCount:1000,forbiddenCalls:[],proof:{freshSave:true,noRecharge:true,noDirectStateMutation:true,legalGameplayCallsOnly:true,ascensionComplete:true,realmIndex:39,lifeCycles:0,totalDeaths:0},final:{realmIndex:39,realm:'真仙',location:'飞升天门',daoPath:dao,ascended:true,tribulationStatus:'complete',thunderStage:6,transformStep:3,lifespan:1000000,age:baseAge[39],manual:manuals[dao],battleWins:100,battleLosses:1,stones:100,rareMaterials:10,originInsight:360,worldAuthority:180,natalMarks:8}};
}
...
try{
 ...
 console.log('V310_V60_PROOF_BINDER_SELFTEST_PASS '+JSON.stringify({validEvidenceAccepted:true,missingGearMarkerRejected:true,invalidShaRejected:true,paths:daos.length,dominanceSyntheticCase:'all-tied'}));
