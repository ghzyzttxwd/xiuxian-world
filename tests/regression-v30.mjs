...
// Stable mirrors stay synchronized when legacy V2.9 gameplay fields change and save.
os.player.manual='太虚化神篇';os.player.realmIndex=23;os.player.location='玄阴禁地';os.player.spells['神念斩']=19;deleteStableFields(os.player);const sd=loadState(os),sa=sd.window.__TAIXUAN_TEST__;sa.syncStableContentState();const ss=sa.getState();assert.equal(ss.player.manualId,'manual-taixu-deification');assert.equal(ss.player.realmId,'realm-deification-early');assert.equal(ss.player.regionId,'region-xuanyin-forbidden');assert.equal(ss.player.spellProficiencyById['spell-divine-thought-slash'],19);
