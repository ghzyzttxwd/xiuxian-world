...
function makeResult(dao,ages,{actions=180000+paths.indexOf(dao)*10000,manual=manuals[dao]}={}){
 const realmLog=realmIndexes.map((realmIndex,i)=>({realmIndex,realm:`R${realmIndex}`,age:ages[realmIndex]??(10+i*10)}));
 return {
  status:'PASS',seed:31006700+paths.indexOf(dao)+1,daoPath:dao,actions,maxRealm:39,
  ...
  final:{realmIndex:39,realm:'真仙',location:'飞升天门',daoPath:dao,ascended:true,tribulationStatus:'complete',thunderStage:6,transformStep:3,lifespan:1000000,age:ages[39],manual,battleWins:100,battleLosses:1,stones:100,rareMaterials:10,originInsight:360,worldAuthority:180,natalMarks:8}
 };
}

function writeCase(root,ageMap,overrides={}){
 fs.mkdirSync(root,{recursive:true});
 for(const dao of paths){
  const dir=path.join(root,dao);fs.mkdirSync(dir,{recursive:true});
  fs.writeFileSync(path.join(dir,'V310_FULLRUN_RESULT.json'),JSON.stringify(makeResult(dao,ageMap[dao],overrides[dao]||{}),null,2)+'\n');
 }
}
function runCase(root){return spawnSync(process.execPath,[aggregator,root],{cwd:process.cwd(),encoding:'utf8'});}
...
function allowedAges(){
 const ages={sword:shifted(0),flame:shifted(5),body:shifted(10),spirit:shifted(15)};
 for(const r of [37,39]){ages.flame[r]=base[r]-1;ages.sword[r]=base[r]}
 return ages;
}
...
try{
 ...
 // FAIL: any route above the legal 300k proof cap must be rejected before balance aggregation.
 const actionDir=path.join(tmp,'fail-action-cap');
 writeCase(actionDir,allowedAges(),{spirit:{actions:300001}});
 proc=runCase(actionDir);
 assert.notEqual(proc.status,0,'aggregate gate incorrectly allowed >300k actions');
 assert((proc.stderr+proc.stdout).includes('action count outside legal proof cap'),'action-cap failure reason missing');

 // FAIL: four paths collapsing onto fewer than three terminal manuals violates build variety.
 const manualDir=path.join(tmp,'fail-manual-collapse');
 writeCase(manualDir,allowedAges(),{
  sword:{manual:'同一终篇'},flame:{manual:'同一终篇'},body:{manual:'第二终篇'},spirit:{manual:'第二终篇'}
 });
 proc=runCase(manualDir);
 assert.notEqual(proc.status,0,'aggregate gate incorrectly allowed fewer than three terminal manuals');
 assert((proc.stderr+proc.stdout).includes('terminal builds collapsed'),'terminal-manual failure reason missing');

 console.log('V310_AGGREGATE_GATE_SELFTEST_PASS '+JSON.stringify({allowedUniqueFastest:5,rejectedUniqueFastest:6,checkpoints:7,actionCapOver300kRejected:true,terminalManualVarietyUnder3Rejected:true}));
}finally{
