...
console.log('V310_REGRESSION_PASS '+JSON.stringify({version:'3.10.0',schema:36,registry:10,realmRateApplied:true,highRealmYears:rows.map(x=>Math.round(x.years)),stableMaterials:reg.counts.materials,stableRegions:reg.counts.regions,factionContractPollutionRemoved:true,preMahayanaGateReachable:true,sectUiHandlersPreserved:true,emptyLivingNpcMinorEventSafe:true,v39SavePreserved:true,futureSaveProtected:true}));
