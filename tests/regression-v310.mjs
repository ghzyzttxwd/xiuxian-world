...
assert(source.includes('"id":"auction-v37-soulcovenant","shopId":"shop-cangwu-auction","kind":"material","refId":"mat-v37-soul-covenant-stone","basePrice":780,"stock":1,"minRealm":29'),'scarce soul-covenant auction source missing');
...
const ud=dom(),ua=ud.window.__TAIXUAN_TEST__;ua.newGame('V310合体道胎回归');ua.v37SetPlayerForTest({realmIndex:29,location:'归一圣墟',lawId:'law-severing',lawProficiency:60,unity:30,insight:100,stones:1000});let seedCount=ua.v33MaterialCount('mat-v37-unity-seed'),normalUnityAttempts=0;while(seedCount<2&&normalUnityAttempts<100){ua.v33AddMaterial('mat-v37-law-crystal',1);ua.v33AddMaterial('mat-v37-soul-covenant-stone',1);const r=ua.v37IntegrateUnity();assert.equal(r?.ok,true,'normal unity integration unexpectedly blocked');normalUnityAttempts++;seedCount=ua.v33MaterialCount('mat-v37-unity-seed')}assert(seedCount>=2,`normal unity integration failed to provide recoverable unity seeds after ${normalUnityAttempts} attempts`);assert(normalUnityAttempts>0,'unity-seed regression did not execute normal integration');

// 元神契石 must also have a scarce economic recovery source at realm29. Test helpers only
// establish realm/location/stones; acquisition itself goes through the normal auction listing
// and normal v35Trade buy path. The lot is stock 1, so this is not a bulk/free resource grant.
const ed=dom(),ea=ed.window.__TAIXUAN_TEST__;ea.newGame('V310元神契石拍卖回归');ea.v35SetPlayerForTest({realmIndex:29,location:'苍梧郡城',stones:100000});let soulLot=null;for(let i=0;i<40&&!soulLot;i++){soulLot=Object.values(ea.v35ListingRegistry()||{}).find(x=>x&&x.shopId==='shop-cangwu-auction'&&x.kind==='material'&&x.refId==='mat-v37-soul-covenant-stone'&&Number(x.stock||0)>0);if(!soulLot)ea.advanceDays(30)}assert(soulLot,'soul-covenant auction lot never appeared');assert.equal(Number(soulLot.stock),1,'soul-covenant auction must remain stock 1');const soulBefore=ea.v33MaterialCount('mat-v37-soul-covenant-stone'),tradeResult=ea.v35Trade(soulLot.id,'buy',1);assert.equal(tradeResult?.ok,true,'normal soul-covenant auction purchase failed');assert.equal(ea.v33MaterialCount('mat-v37-soul-covenant-stone'),soulBefore+1,'soul-covenant auction purchase did not deliver exactly one material');
...
console.log('V310_REGRESSION_PASS '+JSON.stringify({version:'3.10.0',schema:36,registry:10,realmRateApplied:true,highRealmYears:rows.map(x=>Math.round(x.years)),stableMaterials:reg.counts.materials,stableRegions:reg.counts.regions,factionContractPollutionRemoved:true,preMahayanaGateReachable:true,unitySeedSecondarySource:true,unitySeedAttempts:normalUnityAttempts,soulCovenantAuctionSource:true,sectUiHandlersPreserved:true,emptyLivingNpcMinorEventSafe:true,v39SavePreserved:true,futureSaveProtected:true}));
