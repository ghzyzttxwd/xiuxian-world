...
assert.strictEqual(baseline.split(before).length-1,1,'baseline must contain exactly one mismatched V38 natal identity comparison');
run('tools/build-v310-balance-v4.cjs');
const fixed=fs.readFileSync('src/game-v310.js','utf8');
assert.strictEqual(fixed.split(after).length-1,1,'fixed source must contain exactly one corrected V38 natal identity comparison');
assert(!fixed.includes(before),'stale artifactId/natalArtifactId comparison survived');
assert.strictEqual(fixed,baseline.replace(before,after),'V4 must change only the intended V38 natal identity expression relative to V3');
const report=JSON.parse(fs.readFileSync('BUILD_V310_BALANCE.json','utf8'));
assert.strictEqual(report.status,'PASS');assert.strictEqual(report.gameplay_version,'3.10.0');assert.strictEqual(report.build,'31001');assert.strictEqual(report.save_schema_version,36);assert.strictEqual(report.content_registry_version,10);assert(report.changes.some(x=>String(x).includes('natal-artifact combat scaling')),'V4 report must record the identity fix');
console.log('V310_NATAL_ORIGIN_ID_REGRESSION_PASS '+JSON.stringify({singleGameplayDiff:true,identity:'itemId',originMarkCoefficient:0.035,natalSuppressionPenalty:0.20,build:report.build}));
