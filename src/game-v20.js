(() => {
'use strict';
const SAVE_KEY='xiuxian_world_v02'; const OLD_KEY='xiuxian_world_v01'; const VERSION='2.0.0'; const SAVE_SCHEMA_VERSION=17;
...
function validateCurrentSaveSchema(){
 const p=state.player,w=state.world,required=[['player.injury',p.injury],['player.sect',p.sect],['player.alchemyProf',p.alchemyProf],['player.gearOwned',p.gearOwned],['player.breakthroughAttempts',p.breakthroughAttempts],['player.coreEssence',p.coreEssence],['player.coreFailures',p.coreFailures],['player.nascentEssence',p.nascentEssence],['player.nascentFailures',p.nascentFailures],['player.deificationEssence',p.deificationEssence],['player.deificationFailures',p.deificationFailures],['player.rareMaterials',p.rareMaterials],['player.goldenPills',p.goldenPills],['player.soulPills',p.soulPills],['player.dwellingTier',p.dwellingTier],['player.birthDay',p.birthDay],['player.secretRealmClears',p.secretRealmClears],['world.secretRealmCount',w.secretRealmCount],['world.factionClashes',w.factionClashes],['legacy',state.legacy]];
...
function render(){if(!state)return;$('heroName').textContent=state.player.name;$('heroSub').textContent=`${age()}岁 · ${season()} · ${state.player.location} · ${root().name}${state.player.injury?` · ${injuryLabel()}`:''}`;$('realmName').textContent=realm().name;$('cultivationText').textContent=`${Math.floor(state.player.progress)} / ${realm().need}`;$('cultivationBar').style.width=`${clamp(state.player.progress/realm().need*100,0,100)}%`;$('dateTop').textContent=dateLabel();
 ...
 renderUrgent();renderBreakthrough();renderLegacy();renderHomeLog();renderCorePath();renderCharacter();renderSect();renderDwelling();renderMap();renderSecretRealm();renderMarket();renderAlchemy();renderGear();renderEvents();renderFactionConflict();renderNPCs();
}
