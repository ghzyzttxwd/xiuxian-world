...
function normalizeLoaded(){
 if(!state.player.rootIndex&&state.player.rootIndex!==0)state.player.rootIndex=1;if(!state.player.manual)state.player.manual='基础吐纳诀';if(state.player.manualProf==null)state.player.manualProf=0;if(!state.player.spells)state.player.spells={'基础拳脚':0};if(state.player.diligence==null)state.player.diligence=0;if(state.player.insight==null)state.player.insight=0;if(state.player.hp==null)state.player.hp=maxHp();if(state.player.qi==null)state.player.qi=maxQi();if(state.player.injury==null)state.player.injury=0;if(state.player.battleWins==null)state.player.battleWins=0;if(state.player.battleLosses==null)state.player.battleLosses=0;if(state.player.kills==null)state.player.kills=0;if(state.player.sect==null){const joined=!!(state.major?.recruit?.participated&&((ROOTS[state.player.rootIndex]?.mult||0)>=1.25||state.player.realmIndex>=1));state.player.sect=joined?'青云宗':'散修';state.player.sectRank=joined?'外门弟子':'无'}if(state.player.sectRank==null)state.player.sectRank=state.player.sect==='青云宗'?'外门弟子':'无';if(state.player.sectContribution==null)state.player.sectContribution=0;if(state.player.sectTasksCompleted==null)state.player.sectTasksCompleted=0;if(state.player.sectTask==null)state.player.sectTask=null;if(state.player.sectLastStipend==null)state.player.sectLastStipend=0;if(state.player.healingPills==null)state.player.healingPills=0;if(state.player.tradeVolume==null)state.player.tradeVolume=0;if(state.player.alchemyProf==null)state.player.alchemyProf=0;if(state.player.qiPills==null)state.player.qiPills=0;if(state.player.alchemyBatches==null)state.player.alchemyBatches=0;if(state.player.beastMaterials==null)state.player.beastMaterials=0;if(!Array.isArray(state.player.gearOwned))state.player.gearOwned=[];if(!state.player.equipped)state.player.equipped={weapon:null,armor:null,charm:null};for(const k of ['weapon','armor','charm'])if(!(k in state.player.equipped))state.player.equipped[k]=null;if(state.player.breakthroughPity==null)state.player.breakthroughPity=0;if(state.player.breakthroughAttempts==null)state.player.breakthroughAttempts=0;if(state.player.progress>realm().need)state.player.progress=realm().need;if(state.player.dwellingTier==null)state.player.dwellingTier=0;if(state.player.dwellingLocation===undefined)state.player.dwellingLocation=null;if(state.player.herbPlotReady==null)state.player.herbPlotReady=0;if(state.player.herbPlotSeeded==null)state.player.herbPlotSeeded=false;if(state.player.retreatSessions==null)state.player.retreatSessions=0;if(state.player.secretRealmClears==null)state.player.secretRealmClears=0;if(state.player.relicFragments==null)state.player.relicFragments=0;if(state.world.secretRealm===undefined)state.world.secretRealm=null;if(state.world.nextSecretRealmDay==null)state.world.nextSecretRealmDay=dayNumber()+20;if(state.world.secretRealmCount==null)state.world.secretRealmCount=0;if(state.world.factionTension==null)state.world.factionTension=24;if(state.world.factionClashes==null)state.world.factionClashes=0;if(state.world.lastFactionDay==null)state.world.lastFactionDay=0;if(state.world.warWinsQingyun==null)state.world.warWinsQingyun=0;if(state.world.warWinsBlood==null)state.world.warWinsBlood=0;if(state.player.birthDay==null)state.player.birthDay=1;if(!state.legacy)state.legacy={cycles:0,merit:0,bestRealm:0,totalDeaths:0};if(state.legacy.cycles==null)state.legacy.cycles=0;if(state.legacy.merit==null)state.legacy.merit=0;if(state.legacy.bestRealm==null)state.legacy.bestRealm=0;if(state.legacy.totalDeaths==null)state.legacy.totalDeaths=0;
 for(const n of state.npcs||[]){if(n.relation==null)n.relation=0;if(n.grudge==null)n.grudge=0;if(n.known==null)n.known=false;if(n.lastGiftDay==null)n.lastGiftDay=0;if(n.lastDaoDay==null)n.lastDaoDay=0;if(n.lastSparDay==null)n.lastSparDay=0;if(n.lastHelpMonth==null)n.lastHelpMonth=0;if(n.lastRevengeDay==null)n.lastRevengeDay=0}
...
function explore(){advanceDays(1);addDiligence(2);const danger=state.player.location==='黑风岭'?.46:(state.player.location==='临江城'||state.player.location==='青石镇')?.30:.20;const x=rand();if(x<danger){startCombat(weightedEnemy());return}if(x<danger+.30){startMinorEvent();return}if(x<danger+.46){const n=rint(1,4);state.player.spiritStones+=n;addPersonal(`你在一处废弃石缝里发现 ${n} 块灵石。`,'good');save();render();showResult('意外收获',`你找到了一只破布袋。\n灵石 +${n}`,'good');return}addPersonal('你在附近转了一圈，没有碰见特别的事。');save();render();showResult('附近探索','这一日风平浪静。修真界并不是每天都有奇遇。')}
