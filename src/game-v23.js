(() => {
'use strict';
const SAVE_KEY='xiuxian_world_v02'; const OLD_KEY='xiuxian_world_v01'; const VERSION='2.3.0'; const SAVE_SCHEMA_VERSION=20;
...
function realmSuppressionMultiplier(attacker,defender){const diff=(attacker||0)-(defender||0);return diff>=4?1.35:diff>=2?1.18:diff<=-4?.65:diff<=-2?.82:1}
...
function simulateNPCs(){
 for(const n of state.npcs){if(!n.alive)continue;n.progress+=Math.max(1,Math.floor((n.talent/100)*rint(1,4)));if(n.progress>=100&&n.realmIndex<REALMS.length-1){n.progress-=100;n.realmIndex++;if(rand()<.12){n.known=true;addWorld(`${n.name}突破至${REALMS[n.realmIndex].name}。`)}}
