...
function brewAlchemy(id){
 ...
 const success=rand()<before;if(success){const extra=rand()<alchemyGradeChance()?1:0,count=1+extra;if(id==='healing')state.player.healingPills+=count;else state.player.qiPills+=count;state.player.alchemyProf+=7;addPersonal(`你守炉一日，成功炼成【${r.name}】${count}份${extra?'，其中一炉药性格外凝练':''}。炼丹熟练度 +7。`,'good');save();render();showResult('炼丹成功',`【${r.name}】 +${count}\n炼丹熟练度 +7\n当前成功率 ${Math.round(alchemyChance(r)*100)}%`,'good')}
